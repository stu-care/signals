import type { SignalFamily, SignPanel } from '../types'

/*
 * Speed indications — static lineside signs. Each variant is a single flat
 * plate with one aspect describing it. Permanent (PSR) limits and temporary
 * (TSR/ESR) restrictions, in mph.
 */

function sign(s: SignPanel) {
  return [s]
}
  ...
  variants: [
    {
      ...
      panels: sign({ type: 'sign', id: 'psr', kind: 'psr', primary: '40', size: 34, label: 'Permissible speed indicator showing 40' }),
      aspects: [
        {
          id: 'psr',
          ...
          whatYouDo: 'Be at or below the shown speed by the time the front of your train reaches the board.',
          lookAlikes:
            'A single white board with black figures is a permanent limit. Two figures stacked is a differential board; a yellow board with two white spots warns of a temporary restriction.',
    ...
    },
    {
      ...
      panels: sign({ type: 'sign', id: 'diff', kind: 'psr-diff', primary: '60', secondary: '90', size: 30, label: 'Differential speed board showing 60 over 90' }),
      aspects: [
        {
          id: 'psr-diff',
    ...
    },
    {
      ...
      blurb: 'Yellow board with two white spots — warns a temporary restriction is coming.',
      panels: sign({ type: 'sign', id: 'tsrwarn', kind: 'tsr-warn', size: 34, label: 'Temporary speed restriction warning indicator' }),
      aspects: [
        {
          id: 'tsr-warn',
          name: 'TSR warning',
          ...
          concept: 'tsr-warn',
          whatItMeans:
            'A yellow board with two white spots, fitted with an AWS magnet: a temporary speed restriction is coming up, so you get an in-cab warning before the speed indicator.',
          whatYouDo: 'Begin braking so you are at the temporary limit by the commencement (speed) board.',
          safetyInteraction: 'The AWS magnet at the warning board sounds the horn — acknowledge it, then brake for the restriction.',
          controls: 'Acknowledge AWS (default: Q) and brake for the restriction.',
          realWorldNote:
            'Exact warning-board distances are set by the engineering instructions for each restriction; this is a representative depiction.',
    ...
    },
    {
      ...
      panels: sign({ type: 'sign', id: 'tsrspeed', kind: 'tsr-commence', primary: '20', size: 34, label: 'Temporary speed restriction indicator showing 20' }),
      aspects: [
        {
          id: 'tsr-commence',
          name: 'TSR speed',
    ...
    },
    {
      ...
      panels: sign({ type: 'sign', id: 'tsrend', kind: 'tsr-terminate', size: 34, label: 'Temporary speed restriction termination indicator' }),
      aspects: [
        {
          id: 'tsr-terminate',
          name: 'TSR ends',
          ...
          whatYouDo: 'Wait until your entire train is clear of the restriction, then accelerate back to line speed.',
